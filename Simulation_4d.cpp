...
// -----------------------------------------------------------------
// WARNING!  THIS PROGRAM HAS BEEN KNOWN TO SEGFAULT FOR VERY LARGE
// (36^4) LATTICES IN THE WEAKLY COUPLED (lambda = 0.01) STRONGLY
// BROKEN (bimodality = 1) REGIME.  WE SUSPECT THAT THE CLUSTER-
// GROWING RECURSION MAY BE OVERFLOWING THE STACK AND WILL ATTEMPT
// TO ADDRESS THIS IN THE FUTURE BY REPLACING THE RECURSION WITH A
...
int main(unsigned int argc, char** const argv) {
    ...
    unsigned int length = atoi(argv[3]);        // Lattice length (cubic)
    unsigned int latticeSize = int(gsl_pow_4(length));
    ...
    // Now for some autocorrelation and standard deviation
    ...
    if (t == 1) {            // Assume autocorrelation time zero
        autocorTime = 0.0;
        energyStDev = 0.0;
        phiStDev = 0.0;
    }
    ...
    // Take Fourier transform of correlation functions
    // Extrapolate to zero momentum here?
    for (unsigned int i = 0; i < halfL; i++)
        momCorr[i] = posCorr[i];
