...
void Lattice::calcCorrelations(double posCorr[]) {
    ...
    for (unsigned int i = 0; i < latticeSize; i++) {
        ...
        for (unsigned int j = 0; j < halfL; j++) {
            ...
            weight[cur] = 1;
            corr[j] += weight[cur] * lattice[i] * lattice[cur];
            count[j] += weight[cur];
            ...
            for (unsigned int k = 0; k < j; k++) {
                ...
                weight[cur] = weight[neighbors[cur]->prevX]
                            + weight[neighbors[cur]->prevY];
                if (weight[cur] == 0)
                    weight[cur] = 1;
                corr[j] += weight[cur] * lattice[i] * lattice[cur];
                count[j] += weight[cur];
            ...
            for (unsigned int k = 0; k < j; k++) {
                ...
                weight[cur] = weight[neighbors[cur]->nextX]
                            + weight[neighbors[cur]->prevY];
                if (weight[cur] == 0)
                    weight[cur] = 1;
                corr[j] += weight[cur] * lattice[i] * lattice[cur];
