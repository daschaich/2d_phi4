...
#include "Lattice_FFT.hh"           // Method and data declarations
// -----------------------------------------------------------------
...
int main(unsigned int argc, char** const argv) {
    ...
    // Position-space functions constrained to be real
    // Momentum-space functions complex in general
    unsigned int halfL = int(length / 2);
    double posCorr[halfL];
    double momCorr[length];
    ...
    for (unsigned int i = 0; i < sampleSize; i++) {
        ...
        theLattice->calcCorrelations(posCorr, momCorr);
    }
    ...
    for (unsigned int i = 1; i < halfL; i++) {
        ...
        momCorr[i] /= sampleSize;
        posCorr[i] -= avePhi * avePhi;
    }
    for (unsigned int i = halfL; i < length; i++)
        momCorr[i] /= sampleSize;
    ...
    // Only print real components of position-space functions
    for (unsigned int i = 0; i < halfL; i++)
        printf("%lf,", posCorr[i]);

    // Print both real and imaginary parts of the momentum-space functions
    for (unsigned int i = 0; i < length; i++)
        printf("%lf,", momCorr[i]);
    printf("\n");
